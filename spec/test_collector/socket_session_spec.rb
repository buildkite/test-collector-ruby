...
RSpec.describe Buildkite::TestCollector::SocketSession do
  ...
  before do
    ...
    allow(Buildkite::TestCollector::SocketConnection).to receive(:new) { |session, _, _|
      @session = session
  ...
  describe "#write_result" do
    let(:fake_trace) { instance_double("Buildkite::TestCollector::Uploader::Trace") }
  ...
  describe "#disconnected" do
    it "does a reconnect and redoes the socket" do
      ...
      expect(Buildkite::TestCollector::SocketConnection).to have_received(:new).twice
    end
    ...
    it "retries reconnection if it gets a handshake error" do
      ...
      allow(Buildkite::TestCollector::SocketConnection).to receive(:new) { |session, _, _|
        ...
        if call_count.odd?
          ...
          raise Buildkite::TestCollector::SocketConnection::HandshakeError
        end
      }
      ...
      expect(Buildkite::TestCollector::SocketConnection)
    ...
    it "retries reconnection if it gets a socket error" do
      ...
      allow(Buildkite::TestCollector::SocketConnection).to receive(:new) { |session, _, _|
        ...
        if call_count.odd?
          ...
          raise Buildkite::TestCollector::SocketConnection::SocketError
        end
      }
    ...
    it "retries reconnection if it gets a timeout error" do
      ...
      allow(session).to receive(:pop_with_timeout) {
        ...
        case call_count
        when 1
          raise Buildkite::TestCollector::TimeoutError
        when 2
          { "type" => "welcome" }
        when 3
    ...
    it "raises error if it can't reconnect after 3 goes" do
      ...
      expect { session.disconnected(socket_double) }.to raise_error(Buildkite::TestCollector::SocketConnection::SocketError)
      ...
      expect(Buildkite::TestCollector::SocketConnection).to have_received(:new).exactly(5).times
    end
  end
end
